...
def collate_prompt_to_clause_for_causal_lm_loss(batch, tokenizer, topic_sep_token, seq_max_length):
    return_batch = tokenizer(
        [t + topic_sep_token + c + tokenizer.eos_token for t, _, c in batch],
...
if __name__ == '__main__':
    ...
    collate_fn_loss = partial(collate_prompt_to_clause_for_causal_lm_loss, tokenizer=tokenizer, 
        topic_sep_token=TOPIC_SEP_TOKEN, seq_max_length=SEQUENCE_MAX_LENGTH)
    collate_fn_gen = partial(collate_prompt_to_clause_for_causal_lm_generation, tokenizer=tokenizer, 
